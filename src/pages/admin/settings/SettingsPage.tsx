...
import { Loader2, Plus, Building2, FileText, Ruler, Package, Upload } from 'lucide-react'
      ...
      <Card>
        <CardContent className="space-y-4 pt-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            {field('email', 'Email', 'contact@oohmyad.fr')}
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-3">
              ...
              {settings?.logo_path && (
                <span className="text-xs text-muted-foreground">{settings.logo_path}</span>
              )}
              <input
                id="logo-input"
                type="file"
            ...
            {formats?.map((f) => (
              <button
                ...
                onClick={() => {
                  ...
                  updateFormat.mutate({ id: f.id, is_active: active })
                }}
            ...
            {services?.map((s) => (
              <div key={s.id} className="flex items-center gap-3 rounded-md px-3 py-2 hover:bg-muted/50">
                <button
                  ...
                  className={`size-2 shrink-0 rounded-full ${s.is_active ? 'bg-green-500' : 'bg-gray-300'}`}
